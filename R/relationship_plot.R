...
relationship_plot<-function(qtl_file,x,y,grid.col="gray60",degree=90,canvas.xlim=c(-2,2),canvas.ylim=c(-2,2),cex){
  ...
  par(mar=c(0,0,0,0))
  circlize::circos.par(gap.after = c(rep(3, nrow(chord.matrix)-1), length(unique(qtl_file[,x])), rep(3, ncol(chord.matrix)-1), length(unique(qtl_file[,x]))),start.degree = degree, clock.wise = FALSE,canvas.xlim=canvas.xlim,canvas.ylim=canvas.ylim,track.height = 0.5)
